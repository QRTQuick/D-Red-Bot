...
void about_us() {
    ...
    std::cout << COLOR_CYAN << "Software Name: " << COLOR_RESET << "D-Red-Bot (Advanced CLI - Hacker OS Edition)\n";
    std::cout << COLOR_CYAN << "Version: " << COLOR_RESET << "Qv1.5.0\n";
    std::cout << COLOR_CYAN << "Author: " << COLOR_RESET << "Quick Red Tech (Chisom)\n";
    std::cout << COLOR_CYAN << "Description: " << COLOR_RESET << "A futuristic system assistant blending CLI and GUI with predictive memory tracking, live monitoring, and simulated OS boot behavior.\n";
    std::cout << COLOR_CYAN << "Website: " << COLOR_RESET << "https://qrtquick.github.io/QuickRedTechPortfolio/\n";
     std::cout << COLOR_GREEN << "\nNOTE: software only show files in th dir it is in so u should add it in ur folder with the moset files .\n" << COLOR_RESET;
    std::cout << COLOR_GREEN << "\nThank you for using D-Red-Bot — stay sharp, stay red.\n" << COLOR_RESET;
}
void show_menu() {
    while (true) {
        std::cout << COLOR_CYAN << "\nMain Menu:\n" << COLOR_RESET;
